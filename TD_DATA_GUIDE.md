...
毎朝の手動作業は不要です。ビルドスクリプトが tier A 15駅を自動取得します。
...
- tier A 15駅のみ色付き。tier B・未取得駅は ❓（未知ノード）
- キャッシュ: `td_trends_cache.json`（取得失敗時に前回値を使用）
- レート制限対策: 駅ごとに約12秒間隔（全15駅で約3分）
...
- TD > +30% → 🔴 ／ +10〜30% → 🟠 ／ < −10% → 🟢 ／ それ以外・データ薄 → ❓
- Trends: 関心値 0 はデータ薄。Manual: 5件未満はデータ薄
